...
class BEM_test(nn.Module):
    ...
    只使用单个长度的窗，和多尺度窗的结果对比。把Conv2D换为Conv1D
    """
    ...
    def __init__(
        ...
        self.bem_args = config.model.BEM
        ...
        if self.bem_args.use_offset:
            self.reduction_layer_reg = ReductionLayer_test(
                self.hid_dim_1d,
                self.hid_dim_3d,
                self.hid_dim_2d,
                self.num_sample,
        ...
        self.conv_start = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Conv1d(self.hid_dim_2d, out_number, kernel_size=1),
            nn.Sigmoid(),
        )
...
class BEM_Loss_test(nn.Module):
    ...
    def __init__(self, clip_length, index_map, _lambda_1=10, _lambda_2=10, _lambda_3=1):
        super().__init__()
        self.num_classes = config.dataset.num_classes
        self.windows_zize = config.model.BEM.one_window_size # e.g. 16
    ...
    def forward(
        self, conf_dict: Dict[str, Tensor], annos: List[Tensor], *args, **kwargs
    ):
